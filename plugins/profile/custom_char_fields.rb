...
#        return { goals: Website.format_markdown_for_html(char.goals) }
        return { connections: char.bg_shared ? Website.format_markdown_for_html(char.connections) : nil }
      end
...
#        return { connections: Website.format_input_for_html(char.connections) }
      end
...
#        char.update(goals: chargen_data[:custom][:goals])
        ...
        char.update(connections: chargen_data[:custom][:connections])
        return []
      end
